...
export function pickColor({canvasContext, mousePosition, imageSize}: PickColorArguments) {
	return getPixelColor({
		pixelPosition: mousePosition,
		imageSize,
		imageData: canvasContext.getImageData(0, 0, imageSize.width, imageSize.height).data
	});
}
